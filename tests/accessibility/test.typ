...
#context {
  ...
  assert.eq(
    generate-unit-alt-description(
      ..parse-unit("m/s/N").values(),
      value: 1,
    ),
    "metri jaettuna sekunnilla jaettuna newtonilla",
  )
}
...
#context {
  ...
  for (key, words) in units-sl {
    ...
    assert.eq(guad(denom + key, 1), " na " + denom-unit)
  }
}
